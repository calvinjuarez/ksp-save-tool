...
The stack is **Vue 3**, **Vite**, **Pinia**, **Vue Router**, **Bootstrap Reboot** with shared **house** CSS tokens, **PWA** (offline shell), **Vitest**, and optional **GitHub Pages** deploy via Actions. Uploading an `.sfs` file parses it in the browser and shows a short summary on Home; open **Crew manifest** for a full table and markdown export (rescue and tourist marks from `ContractSystem` and roster).
